...
class Smarty_Compiler_Switch extends Smarty_Internal_CompileBase
{
    ...
    */
   protected function close_open_statements($compiler)
   {
      $counter = 0;
      while (count($this->compiler->_tag_stack))
      {
         ...
         {
            $this->closeTag($compiler, array('case', 'default'));
            $counter++;
         }
         else {
            break;
   ...
   public function compile($args, $compiler)
   {
      ...
      $this->openTag($compiler, 'switch', true);
...
class Smarty_Compiler_Switchclose extends Smarty_Compiler_Switch
{
   ...
   public function compile($args, $compiler)
   {
      ...
      $this->close_open_statements($compiler);
      $this->closeTag($compiler, 'switch');
...
class Smarty_Compiler_Break extends Smarty_Compiler_Switch
{
   ...
   public function compile($args, $compiler)
   {
      ...
      if ($this->close_open_statements($compiler)) {
         return '<?php break; ?>';
      }
      return '';
   }
...
class Smarty_Compiler_Case extends Smarty_Compiler_Switch
{
   ...
   public function compile($args, $compiler)
   {
      ...
      else
      {
         ...
         $this->closeTag($compiler, 'switch');
         $this->openTag($compiler, 'switch', false);
      }
      ...
      $this->openTag($compiler, 'case', $break);
...
class Smarty_Compiler_Default extends Smarty_Compiler_Switch
{
   public function compile($args, $compiler)
   {
      ...
      $this->openTag($compiler, 'default', $break);
