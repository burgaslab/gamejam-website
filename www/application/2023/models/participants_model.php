...
class Participants_model extends CI_Model {
	...
	public function add($model) {
		...
		$sql = "INSERT INTO participants (year, name, email, age, occupation, skills, time_confirmed, team_id, shirt_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public function update($id, $model) {
		...
		if ($current) {
			...
			$sql = "UPDATE participants SET name=?, email=?, age=?, occupation=?, skills=?, time_confirmed=?, team_id=?, shirt_size=? WHERE id=? AND year=?";
			$this->db->query($sql, $data);
	...
	public function validate($id, $data, $age=true) {
		$res = new validator();
		...
		if (!in_array($data["shirt_size"], conf("shirt_sizes"))) {
			$res->add_error("shirt_size", "Невалиден размер на тениска");
		}
		return $res;
	}
}